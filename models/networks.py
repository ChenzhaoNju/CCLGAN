...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'resnet_9blocks':
        ...
        net = RiTGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                           no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=6, opt=opt)
    elif netG == 'ritt':
        net = RiTTGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                            no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=6, opt=opt)
...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False,
    ...
    if netF == 'global_pool':
        ...
        net = PatchSampleNonlocalOneGroup(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids,
                                          nc=opt.netF_nc)
...
class PatchSampleNonlocalOneGroup(nn.Module):
    ...
    def create_mlp(self, feats):
        ...
        self.mlp_init = True
        # return init_net(self, self.init_type, self.init_gain, self.gpu_ids)
    ...
    def forward(self, feats):
        ...
        #    return_feats=self.create_mlp(feats)
        return self.create_mlp(feats)
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False,
    ...
    elif netD == 'mix':  # more options
        net = mixDiscriminator(input_nc, ndf, n_layers_D, norm_layer=norm_layer, no_antialias=no_antialias, )
...
class CCB(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.block7 = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=7, padding=3, stride=1,
                      bias=False),
            nn.BatchNorm2d(out_channels))
        self.attn = nn.Sequential(FcaLayer(out_channels, 8, 32, 32))
...
class TransBlosck(nn.Module):
    def __init__(self, norm_layer, use_bias, img_size=8, patch_size=8, embed_C=1024, feat_C=1024, depth=3, heads=4,
                 mlp_dim=4096):
        super(TransBlosck, self).__init__()
        ...
        model = [nn.Conv2d(256, 512, kernel_size=3,
                 ...
                 norm_layer(512),
                 nn.ReLU(True)]
        model += [nn.Conv2d(512, 1024, kernel_size=3,
                            stride=2, padding=1, bias=use_bias),
                  norm_layer(1024),
                  nn.ReLU(True)]
        ...
        # Upsampling block
        model = [nn.ConvTranspose2d(1024, 1024,
                                    kernel_size=3, stride=2,
                                    padding=1, output_padding=1,
                                    bias=use_bias),
                 norm_layer(1024),
                 nn.ReLU(True)]
        model += [nn.ConvTranspose2d(1024, 512,
                                     kernel_size=3, stride=2,
                                     padding=1, output_padding=1,
                                     bias=use_bias),
                  norm_layer(512),
        ...
        self.pos_embed = F.interpolate(pos_embed.permute(0, 3, 1, 2), size=(8, 8), mode='bilinear').flatten(
            2).transpose(-1, -2)
        self.transformer = AdaIn_Transformer(embed_C, depth, heads, feat_C, mlp_dim, dropout=0.)
        # self.cc = channel_compression(512,256)
        self.cc = CCB(512, 256)
    ...
    def forward(self, x, box_info=None, num_box=-1):
        ...
        # print(self.patch_embed(x).shape)
        # print(embed_x .shape)
...
class RiTGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        """
        assert (n_blocks >= 0)
        super(RiTGenerator, self).__init__()
        self.opt = opt
        ...
        input_dim = ngf * mult * 2
        img_size = 256
        model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                              use_bias=use_bias)]
        model += [TransBlosck(norm_layer=norm_layer, use_bias=use_bias)]
        model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                              use_bias=use_bias)]
        # model += [TransBlosck(norm_layer=norm_layer,use_bias=use_bias)]
        model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
...
class RiTTGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        """
        assert (n_blocks >= 0)
        super(RiTTGenerator, self).__init__()
        self.opt = opt
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            if (no_antialias):
                ...
                sequence += [
                    #nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, dilation=1 * nf_mult,
...
class FPNBlock(nn.Module):
    ...
    def __init__(self, ndf, nf_mult_prev, nf_mult, kw, padding, bias, norm_layer):
        """Initialize the Resnet block
        ...
        self.conv_block = self.build_conv_block(ndf, nf_mult_prev, nf_mult, kw, padding, bias, norm_layer)
        self.conv1 = nn.Sequential(
            nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=1, stride=1, padding=padding - 1, bias=bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True),
            Downsample(ndf * nf_mult),
            # FcaLayer(ndf * nf_mult, 8, 32, 32)
            )

    def build_conv_block(self, ndf, nf_mult_prev, nf_mult, kw, padding, bias, norm_layer):
        """Construct a convolutional block.
        ...
        conv_block = []
        conv_block += [
            nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, dilation=nf_mult + nf_mult_prev,
                      padding=nf_mult + 3 * nf_mult_prev - 1, bias=bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True),
            Downsample(ndf * nf_mult)]
    ...
    def forward(self, x):
        """Forward function (with skip connections)"""
        out = self.conv1(x) + self.conv_block(x)  # add skip connections
        # print('*******')
        # print(x.shape)
        # out = self.conv1(x)
        # print(out.shape)
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            if (no_antialias):
                ...
                sequence += [
                    FPNBlock(ndf=ndf, nf_mult_prev=nf_mult_prev, nf_mult=nf_mult, kw=kw, padding=padw, bias=use_bias,
                             norm_layer=norm_layer)]
